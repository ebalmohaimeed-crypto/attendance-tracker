...
// ===================== PERSISTENCE =====================
function saveState() {
  localStorage.setItem("att_v6", JSON.stringify(state));
}
function loadState() {
  const raw = localStorage.getItem("att_v6");
...
function openWeekModal(num) {
  ...
  state.members.forEach(m => {
    ...
    const lectureEntries = sched.map((s, idx) => {
      ...
      const val = key in wData ? wData[key] : undefined;
      return { s, key, val };
    });
...
function renderAttendance() {
  ...
  // استعادة حالة الإجازة
  const hol    = state.holidays["week" + w] || {};
  const chkEl  = document.getElementById("chk-holiday");
  const noteEl = document.getElementById("holiday-note");
  chkEl.checked          = !!hol.isHoliday;
  ...
  // ── نجمع كل المحاضرات مع مفاتيحها ──────────────────────────────────
  // كل عنصر = { m, s, gi } حيث gi = الفهرس العالمي للمحاضرة في جدول العضو
  const lectures = [];
  state.members.forEach(m => {
    (m.schedule || []).forEach((s, gi) => {
      // فلتر اليوم: إذا كان "الكل" نعرض الجميع، وإلا نعرض يوم المحاضرة فقط
      if (dayF !== "all" && s.day !== dayF) return;
      lectures.push({ m, s, gi });
    });
  });

  if (!lectures.length) {
    list.innerHTML = '<div class="empty-state">لا توجد محاضرات لهذا اليوم</div>';
    return;
  }
  ...
  lectures.forEach(({ m, s, gi }) => {
    // ── مفتاح فريد لهذه المحاضرة: memberId_gi ──
    const key      = m.id + "_" + gi;
    const saved    = weekData[key];        // true | false | undefined
    const checked  = saved !== false;      // افتراضي حاضر ما لم يُسجَّل غياب
    const isHol    = !!hol.isHoliday;

    // عنوان اسم العضو فاصل عند أول محاضرة له
    if (m.id !== lastId) {
      html += '<div class="attend-member-name">' + m.name + '</div>';
      lastId = m.id;
    }

    html +=
      '<div class="attend-lecture-row' + (isHol ? " holiday-row" : "") + '">' +
        '<label class="toggle">' +
          '<input type="checkbox"' +
          ' data-key="' + key + '"' +
          (checked ? ' checked' : '') +
        ...
        '</div>' +
        '<span class="badge ' +
          (isHol ? "badge-warn" : checked ? "badge-success" : "badge-danger") +
          '" id="ab-' + key + '">' +
          (isHol ? "إجازة" : checked ? "حاضر" : "غائب") +
        '</span>' +
      '</div>';
  });
